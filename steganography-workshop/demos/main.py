...
def read_root():
    ...
    <html>
        ...
        <body>
             <h1><a href="http://0.0.0.0:5555/docs"> Access the DEMO </a></h1>
        </body>
    </html>
    """
...
def encoded_predict(file_source: UploadFile, file_payload: UploadFile):
    encoded_result_path, _ = inference_encoder(
        model=model_pretrained,
        source_img=file_source.file,
        Payload_img=file_payload.file,
        device="cpu",
    )
...
def decode_predict(file_encoded: UploadFile):
    decoded_result_path, _ = inference_decoder(
        model=model_pretrained,
        image_to_decode=file_encoded.file,
        device="cpu",
    )
